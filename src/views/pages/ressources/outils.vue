            ...
            <v-row align="center" justify="center">
              <v-col cols="12" class="text-center">
                ...
                <p class="text-subtitle-1 font-weight-regular text-grey-darken-1 mb-8 mx-auto"
                 ...
                 Découvrez les dernières innovations et réflexions en matière de ressources humaines, et engagez-vous dans des discussions enrichissantes avec vos collègues.
                </p>
              </v-col>
            </v-row>
          </v-container>
        ...
        <section class="pb-4 pt-2">
          <v-container class="nav-container">
            <div class="d-flex justify-center flex-wrap">
              ...
              <v-btn v-for="cat in filteredCategories" :key="cat.id" variant="tonal" 
                color="primary" class="rounded-pill mx-1 my-1" 
          ...
          <v-row v-if="filteredArticles.length > 0" style="width: 100%; overflow: hidden;">
            ...
            <v-col cols="12" md="9">
              <v-card v-if="featuredArticle" elevation="0" class="rounded-lg overflow-hidden article-card"
                ...
                <div class="featured-media-container position-relative">
                  <MediaDisplay
                    :media-file="featuredArticle.featured_image"
                    :height="500"
                    image-class="rounded-lg article-image"
                  />
                  <div class="article-overlay d-flex flex-column justify-end h-100 position-absolute top-0 left-0 w-100">
                    <div class="pa-4">
                ...
                <v-col cols="12" v-for="article in recentArticles.slice(0, 2)" :key="article.id">
                  <v-card class="article-card" flat @click="navigateToArticle(article)">
                    <div class="d-flex flex-column rounded-lg">
                      <MediaDisplay
                        :media-file="article.featured_image"
                        :height="125"
                        image-class="rounded-t-lg"
                      />
                      <v-card-text class="px-1 pt-2">
                        <v-chip :color="getCategoryVuetifyColor(article.category)" size="small" class="mb-3 category-chip" variant="flat">
                          <span class="text-caption">{{ getCategoryName(article.category) }}</span>
                        </v-chip>
          ...
          <div v-if="filteredArticles.length === 0" class="text-center py-8">
            <v-card flat class="pa-8">
              ...
              <p class="text-grey-darken-1">
                {{ searchQuery ? 'Aucun article ne correspond à votre recherche.' : 'Aucun article disponible dans cettecatégorie.' }}
              </p>
...
// Catégories filtrées (afficher seulement les catégories 3, 4, 5 et 7)
const filteredCategories = computed(() => {
  return categories.value.filter((cat, index) => {
    // Inclure seulement les positions 3, 4, 5 et 7 (index + 1 = position)
    return [3, 4, 5, 7].includes(index + 1);
  });
});
...
const filteredArticles = computed(() => {
  ...
  // Toujours filtrer par les catégories autorisées (3, 4, 5, 7)
  filtered = filtered.filter(article => {
    ...
    // Chercher l'objet catégorie correspondant
    const catObj = categories.value.find(c => c.id === articleCatId);
    if (!catObj) return false;
    
    // Trouver l'index de cette catégorie dans la liste complète
    const categoryIndex = categories.value.findIndex(c => c.id === catObj.id);
    
    // Inclure seulement les positions 3, 4, 5 et 7 (index + 1 = position)
    return [3, 4, 5, 7].includes(categoryIndex + 1);
  });
  
  // Filtrer selon le tab actif (slug de catégorie) seulement si activeTab n'est pas null
  ...
  // Si activeTab est null, on affiche tous les articles des catégories autorisées (3, 4, 5, 7)
...
const featuredArticle = computed(() => {
  // Utiliser le premier article des articles filtrés (qui sont triés par date décroissante)
  const article = filteredArticles.value[0] || null;
  if (article) {
...
const getCategoryColor = (category: any) => {
  ...
  // Couleurs spécifiques selon le nom de la catégorie
  const categoryName = getCategoryName(category);
  const colorMap: Record<string, string> = {
    'Livres': '#006FFF',
    'Podcast': '#EE82EE',  
    'Podcasts': '#EE82EE', // Au cas où le nom serait au pluriel
    'Portraits': '#008000',
    'TED Talks': '#FF0000' // Ajout de TED Talks depuis votre exemple
  };
  
  return colorMap[categoryName] || 'primary';
};

// Fonction pour obtenir la couleur CSS compatible Vuetify
const getCategoryVuetifyColor = (category: any) => {
  const hexColor = getCategoryColor(category);
  
  // Conversion des couleurs hex vers des noms Vuetify ou retour de la couleur hex
  const vuetifyColorMap: Record<string, string> = {
    '#006FFF': 'blue',
    '#EE82EE': 'purple-lighten-2', 
    '#008000': 'green',
    '#FF0000': 'red'
  };
  
  return vuetifyColorMap[hexColor] || hexColor;
};
...
.featured-media-container .media-display {
  height: 100%;
  width: 100%;
}
